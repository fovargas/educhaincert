...
did
    .command('generate')
    .description('create an identifier')
    .option('-i, --identifier-provider <provider>', 'Specify the identifier provider')
    .option('-k, --kms <kms>', 'Specify the key management system')
    .option('-a, --alias <alias>', 'Specify the alias')
    .action(async (opts, cmd) => { // Aquí se cambió 'opts, cmd' por solo 'opts'
        const agent = await getAgent(cmd.optsWithGlobals().config); // Usar 'opts' para obtener la configuración
  
        try {
            // Usa los valores de las opciones o valores predeterminados
            const provider = opts.identifierProvider || 'default-provider';
            const kms = opts.kms || 'default-kms';
            const alias = opts.alias || 'default-alias';
  
            // Ahora puedes usar provider, kms y alias directamente
            const identifier = await agent.didManagerCreate({ provider, kms, alias });
