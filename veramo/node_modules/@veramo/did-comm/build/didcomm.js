import { createJWE, decryptJWE, verifyJWS, } from 'did-jwt';
import { parse as parseDidUrl, } from 'did-resolver';
import { a256cbcHs512AnonDecrypterX25519WithA256KW, a256cbcHs512AnonEncrypterX25519WithA256KW, a256cbcHs512AuthDecrypterX25519WithA256KW, a256cbcHs512AuthEncrypterX25519WithA256KW, a256gcmAnonDecrypterX25519WithA256KW, a256gcmAnonEncrypterX25519WithA256KW, a256gcmAuthDecrypterEcdh1PuV3x25519WithA256KW, a256gcmAuthEncrypterEcdh1PuV3x25519WithA256KW, xc20pAnonDecrypterX25519WithA256KW, xc20pAnonEncrypterX25519WithA256KW, xc20pAuthDecrypterEcdh1PuV3x25519WithA256KW, xc20pAuthEncrypterEcdh1PuV3x25519WithA256KW, } from './encryption/a256kw-encrypters.js';
import { a256cbcHs512AnonDecrypterX25519WithXC20PKW, a256cbcHs512AnonEncrypterX25519WithXC20PKW, a256cbcHs512AuthDecrypterX25519WithXC20PKW, a256cbcHs512AuthEncrypterX25519WithXC20PKW, a256gcmAnonDecrypterX25519WithXC20PKW, a256gcmAnonEncrypterX25519WithXC20PKW, a256gcmAuthDecrypterEcdh1PuV3x25519WithXC20PKW, a256gcmAuthEncrypterEcdh1PuV3x25519WithXC20PKW, xc20pAnonDecrypterX25519WithXC20PKW, xc20pAnonEncrypterX25519WithXC20PKW, xc20pAuthDecrypterEcdh1PuV3x25519WithXC20PKW, xc20pAuthEncrypterEcdh1PuV3x25519WithXC20PKW, } from './encryption/xc20pkw-encrypters.js';
import schema from './plugin.schema.json' assert { type: 'json' };
import { v4 as uuidv4 } from 'uuid';
import { createEcdhWrapper, extractManagedRecipients, extractSenderEncryptionKey, mapRecipientsToLocalKeys, } from './utils.js';
import { bytesToUtf8String, decodeJoseBlob, dereferenceDidKeys, encodeJoseBlob, hexToBytes, isDefined, mapIdentifierKeysToDoc, resolveDidOrThrow, stringToUtf8Bytes, } from '@veramo/utils';
...
export class DIDComm {
     ...
     */
    constructor(transports = [new DIDCommHttpTransport()]) {
    ...
    async packDIDCommMessageJWE(args, context) {
        ...
        let recipients = [];
        async function computeRecipients(to) {
            // 2.1 resolve DID for "to"
            const didDocument = await resolveDidOrThrow(to, context);
            // 2.2 extract all recipient key agreement keys and normalize them
            const keyAgreementKeys = (await dereferenceDidKeys(didDocument, 'keyAgreement', context))
            ...
            if (options.enc === 'A256GCM') {
                if (args.packing === 'authcrypt' && (!options.alg || options.alg?.startsWith('ECDH-1PU'))) {
                    if (options.alg?.endsWith('+XC20PKW')) {
                        ...
                        return a256gcmAuthEncrypterEcdh1PuV3x25519WithXC20PKW(recipient.publicKeyBytes, senderECDH, {
                            kid: recipient.kid,
                        });
                    }
                    else if (options?.alg?.endsWith('+A256KW')) {
                        // FIXME: the didcomm spec actually links to ECDH-1PU(v4)
                        return a256gcmAuthEncrypterEcdh1PuV3x25519WithA256KW(recipient.publicKeyBytes, senderECDH, {
                            kid: recipient.kid,
                        });
                    }
                }
                ...
                if (args.packing === 'authcrypt' && (!options.alg || options.alg?.startsWith('ECDH-1PU'))) {
                    if (options.alg?.endsWith('+XC20PKW')) {
                        // FIXME: the didcomm spec actually links to ECDH-1PU(v4)
                        return a256cbcHs512AuthEncrypterX25519WithXC20PKW(recipient.publicKeyBytes, senderECDH, {
                            kid: recipient.kid,
                        });
                    }
                    else if (options?.alg?.endsWith('+A256KW')) {
                        // FIXME: the didcomm spec actually links to ECDH-1PU(v4)
                        return a256cbcHs512AuthEncrypterX25519WithA256KW(recipient.publicKeyBytes, senderECDH, {
                            kid: recipient.kid,
                        });
                    }
                }
                ...
                if (args.packing === 'authcrypt' && (!options.alg || options.alg?.startsWith('ECDH-1PU'))) {
                    if (options.alg?.endsWith('+XC20PKW')) {
                        // FIXME: the didcomm spec actually links to ECDH-1PU(v4)
                        return xc20pAuthEncrypterEcdh1PuV3x25519WithXC20PKW(recipient.publicKeyBytes, senderECDH, { kid: recipient.kid });
                    }
                    else if (options?.alg?.endsWith('+A256KW')) {
                        // FIXME: the didcomm spec actually links to ECDH-1PU(v4)
                        return xc20pAuthEncrypterEcdh1PuV3x25519WithA256KW(recipient.publicKeyBytes, senderECDH, {
                            kid: recipient.kid,
                        });
                    }
                }
    ...
    async unpackDIDCommMessageJWS(jws, context) {
        // TODO: currently only supporting one signature
        const signatureEncoded = isDefined(jws.signature)
            ? jws.signature
            : jws.signatures[0]?.signature;
        const headerEncoded = isDefined(jws.protected)
            ? jws.protected
        ...
        const message = decodeJoseBlob(jws.payload);
    ...
    async unpackDIDCommMessageJWE({ jwe }, context) {
        // 0 resolve skid to DID doc
        //   - find skid in DID doc and convert to 'X25519' byte array (if type matches)
        let senderKeyBytes = await extractSenderEncryptionKey(jwe, context);
        ...
        for (const localKey of localKeys) {
            ...
            let decrypter = null;
            const recipientECDH = createEcdhWrapper(localKey.localKeyRef, context);
    ...
    decodeMessageAndMediaType(message) {
        ...
        let mediaType = null;
        if (msgObj.typ === DIDCommMessageMediaType.PLAIN) {
    ...
    async sendDIDCommMessage(args, context) {
        ...
        const services = didDoc.service?.filter((service) => service.type === 'DIDCommMessaging');
        ...
        if (isServiceEndpointDid) {
            ...
            return this.sendDIDCommMessage({ packedMessage: wrappedMessage, recipientDidUrl: serviceEndpointUrl, messageId }, context);
        }
